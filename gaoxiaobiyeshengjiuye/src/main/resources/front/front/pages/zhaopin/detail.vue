		...
		<view class="bottom-content cu-bar bg-white tabbar border shop">
			<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
				...
				<view @tap="fanhui" style="display: flex; width: 80%;color: #FFFFFF;">
					<view :style="[{backgroundImage:'linear-gradient(120deg,'+ btnColor[0] +','+ btnColor[1] +')'}]"
						style="width: 95%;height: 80rpx;line-height: 80rpx;text-align: center;border-radius: 40rpx;">
						申请面试
					</view>
				</view>
				<!--两个按钮
		...
		<uni-popup ref="deliver" type="center">
			<view style="width: 500rpx;background-color: #FFFFFF;height: auto;border-radius: 20rpx;padding-top: 20rpx;">
				...
				<view style="width: 90%;margin: 0 auto;padding-top: 10rpx;">
					...
					<textarea v-model='commentData' placeholder="留言内容"
						style="
				...
				<view style="display: flex;width: 100%;margin: 20rpx auto;border-top: 1rpx solid #dddddd;">
					...
					<view @tap="submitData"
						style="width: 50%;border-left: 1rpx solid #dddddd;text-align: center;height: 75rpx;line-height: 75rpx;color: #0081FF;">
						确认
					</view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			//返回
			async fanhui() {
				uni.setStorageSync("zhaopinId", this.detail.id);
				this.$utils.jump("/pages/mianshishenqing/add-or-update")
			},
			//下载
			download(url) {
				let _this = this;
			...
			async init() {
				...
				this.detail.zhaopinContent = this.detail.zhaopinContent.replace("<img", "<img  class=\"daxiao\"")
				...
				if (res.data.list.length == 1) {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.del('zhaopinCollection', JSON.stringify([
									zhaopinCollectionId
								]));
