	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('mianshishenqing','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.mianshishenqingName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('mianshishenqing','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                 <view v-show="item.mianshishenqingTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">面试时间</text>
                     <text style="width: 70%">{{item.mianshishenqingTime}}</text>
                 </view>
                 <view v-show="item.mianshishenqingYesnoValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">申请状态</text>
                     <text style="width: 70%">{{item.mianshishenqingYesnoValue}}</text>
                 </view>
                 <view v-show="item.mianshishenqingYesnoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">申请回复</text>
                     <text style="width: 70%">{{item.mianshishenqingYesnoText}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 12rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('mianshishenqing','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
            </view>
        </view>
    </view>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let mianshishenqingYesnoTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'mianshishenqing_yesno_types'
		  	});
		  	this.mianshishenqingYesnoTypesList = mianshishenqingYesnoTypes.data.list;
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`mianshishenqing`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('mianshishenqing', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
                ...
                let res = await this.$api.page(`mianshishenqing`, searchForm);
				// 如果是第一页数据置空
